...
- A generated file may now declare a `create-only` write policy: it is written when absent and never again, with or without `--force`, and `inspect` and `init` report it as `preserved`. `.ai/tasks/observations.yaml` is the only file that carries it, because from the first completed task it holds promoted and permanently declined observations that only the user and the orchestrator may change, and `syn init --force` is the documented migration command — it must not be the command that erases them. `.ai/tasks/.gitignore` stays refreshable: it is canonical policy with no user state in it.
- Activating a generated project skill is two writes and nothing else: the implementer sets `status: active` in the skill's own frontmatter, which is what `doctor` counts against the twelve-skill budget, and the orchestrator sets the observation to `status: promoted` with `promoted_to`. There is no registry entry, because `.ai/projects/<id>.skills.yaml` has no project-skill slot, is regenerated by every `sync`, and lies outside the orchestrator's `.ai/tasks/**` write scope. Retirement mirrors it: `RETIRED.md` plus `status: retired`, then the ledger. `doctor` enforces the ledger-to-skill direction as `generated.broken-promotion-link`.
- The Canonical Skill Contract is enforced on the canonical base skills under `.ai/skills/<id>/` and the generated project skills under `.ai/skills/project/<id>/`. `.ai/skills/technology/**` and the bundled Ingenium library are exempt by design: they are imported at the `reference-only` trust level and rewriting them to this contract would break their provenance.
- Per-layer size overruns are warnings in both namespaces, including a generated skill's `references/` files (`generated.reference-size`). Both namespaces call one `checkByteCeiling` helper so the severities cannot diverge. The 15KB ceiling on a generated `SKILL.md` stays a blocking error, because it is a contract rule (design §5 rule 4) rather than a budget.
