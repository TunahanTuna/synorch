...
> Status: First slice implemented and integrated — observation ledger, generated-skill contract and its doctor checks, the `skill-creator` base skill, the `sync` protections, and the wiring into `doctor-service.ts` and `createStructureFiles`.
> Decided: 2026-09-22
...
independent reviewer verifies claims against sources
        ↓
skill written to .ai/skills/project/<id>/SKILL.md with status: active,
and the observation moved to status: promoted with promoted_to
```
...
Retirement moves the file to `.ai/skills/project/<id>/RETIRED.md` and sets `status: retired` in its frontmatter, which is what frees the budget slot. There is no registry entry to drop: `.ai/projects/<id>.skills.yaml` has no project-skill slot and is regenerated by every `sync`. Nothing is deleted from Git history.
