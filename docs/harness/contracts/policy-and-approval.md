...
`schema_version`, `policy_version`, `mode`, `role`, `run_id`, `task_id?`, `workspace_root` (attempt'in izole kökü olabilir), `write_scope`, `read_scope`, `forbidden`, `effects{read, workspace-write, exec, external-write, control}`, `external_write_allowlist`, `network{mode: deny|allowlist|allow, hosts}`, `sandbox{backend, enforcement}`, `require_full_sandbox`, `exec_confinement?` (`full-sandbox|allowlist|ask`), `verification_commands?` (varsayılan `[]`), `workspace_trusted?` (v3, yoksa güvenilmez), `dependency_links?` (attempt worktree'sine ana ağaçtan bağlanan bağımlılık dizinleri, ADR-19; yoksa hiç), `layers[]{layer, source, digest}`.

**Bağlı bağımlılıklar (`dependency_links`, bağımsız inceleme R3).** Bağlı dizinler `forbidden`'da da yer alır (dosya araçları okumaz, yazmaz; bağlantıdan geçen yol zaten `link-escape`). Liste boş değilken bağımlılık kuran/ekleyen/kaldıran/güncelleyen her komut (`npm/pnpm/yarn/bun install|i|add|remove|update|ci|…`, çıplak `yarn`, `pip install`, `python -m pip install`, `python -m venv`, `uv sync|add|remove|pip install`, `poetry install|add|…`, `pipenv install`, `cargo fetch|add|update|install|vendor`, `go get|install|mod download|tidy|vendor`, `dotnet restore|add|remove`, `composer/bundle/gem install|…`) her sandbox düzeyinde ve kabuk/önek sarmalayıcılarının içinden reddedilir: kod `dependency-mutation-in-linked-worktree`, hard rail `write-outside-scope` (gateway'de `path_outside_scope`). Kural hem worker `exec`'ine hem harness doğrulamasına uygulanır. Güvenilen build/test'in bağlı dizinler altına yazdığı önbellekler kalan risktir (ADR-19).
