...
- `ContextBuildInput.reportOnly?: string` / `TurnInput.reportOnly?` (canlı çalıştırma 01M37V2J): adım sınırını raporsuz dolduran attempt'in zorunlu son turu. Builder yalnız o rapor aracını sunar ve bütçe kapısını `RequestBudgetGate.admit(observations, { grace: true })` ile çağırır (kapı yalnız adım limiti doluysa bu tek isteği kabul eder, adım saymaz; maliyet/wall-time asla); driver başka araç çağrısını gateway'e göndermeden "not executed: this is a report-only turn" hata sonucuyla yanıtlar. Yalnız worker manager ayarlar.
