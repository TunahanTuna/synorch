...
- Bu yüzden tam olmayan sandbox'ta depo kodunu çalıştıran komutlar **çalışma alanı güveni** ister (SEC-N1): kullanıcı çalışma alanına bir kez, yalnız kullanıcı kapsamında (`<synorch home>/trust.json`; kanonik kök + depo kimliği) güvenir — `syn trust`, etkileşimli tek seferlik soru ("Not now" önceden seçili; "Trust for this session only" kalıcı değildir, "Trust this workspace" kalıcıdır) veya tek run için `--trust-workspace`. Soru ve `syn trust` açıkça söyler: güvenilen çalışma alanında test/build betikleri **ve oturum sırasında yapay zekânın yazdığı her kod** kullanıcının izinleriyle çalışır ve çalışma alanı dışındaki dosyalara, Synorch kimlik bilgileri dahil, erişebilir; çünkü bu platformda sandbox tam değildir. Program araması PATH'teki çalışma alanı veya Synorch home içi girdileri atlar (S1). Güvenilmeyen çalışma alanında `autonomous` mod bu komutları `workspace-untrusted` ile reddeder, `ask` modu sorar, headless run exit 3 ile durur. Depo içeriği güven veremez. Ayrıntı: [policy ve onay §4](../contracts/policy-and-approval.md#4-effectivepolicy-alanları).
