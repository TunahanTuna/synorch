...
- Git'in ignore ettiği bağımlılık dizinleri (`DEPENDENCY_LINK_DIRECTORIES`: `node_modules`, `.venv`, `venv`, `.tox`) ana ağaçta varsa ve hiçbir owned path ile örtüşmüyorsa worktree'ye junction/symlink ile bağlanır; policy bunları salt okunur tutar.
...
- Bağımlılık bağlantıları worktree'den ana ağacın `node_modules`'una yazma yolu açar; policy salt okunur tutar, `exec` ile oraya yazan bir komut (ör. `npm install`) ana ağacı etkileyebilir — doğrulama komutlarının bağımlılık kurmaması beklenir, ADR-06 sandbox kısıtı geçerlidir.
