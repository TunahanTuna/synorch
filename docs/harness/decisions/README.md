...
> Durum: normatif; ADR'ler `Accepted`. Tarih: 2026-09-22. `Accepted` seçeneğin gerekçeyle seçildiğini gösterir, kodun yayımlandığı anlamına gelmez; `Implemented` statüsü yalnız kod/test kanıtıyla verilir. Yaşam döngüsü (`Proposed → Accepted → Implemented → Superseded | Rejected`) ve değişiklik kuralları: [yönetişim](../workflow/governance.md) §3. Karar kuyruğunun tarihçesi: [açık kararlar](../delivery/decisions.md).
...
| ADR-07 | Worker izolasyonu | Accepted | 2026-09-22 | [ADR-07](./ADR-07-worker-isolation.md) | [task-packets](../contracts/task-packets.md) | HREQ-015 |
...
| ADR-09 | Reviewer bağımsızlığı | Accepted | 2026-09-22 | [ADR-09](./ADR-09-reviewer-independence.md) | [task-packets](../contracts/task-packets.md) | HREQ-004 |
