...
| completion `failed` | `running → failed → retry_pending → ready`, delta packet ile yeni attempt (`maxRetries`); delta önceki raporu taşır (`retryNotes`). Retry'dan önce route yeniden çözülür: kota bitip bloklanan route (`RouteBlockedFailure`) sessizce değiştirilmez; router'ın `provider-change` önerisi `approval/requested` + `approval/decided` olarak kaydedilir ve yalnız kullanıcı onayıyla uygulanır, aksi halde görev `blocked` olur. |
...
**Triyaj (`partial` / worker `needs_context`).** Aynı paketle tekrar denemek aynı sonucu verir (ilk canlı çalıştırmada explorer iki kez `partial` döndü). Bunun yerine `Planner.triage` orchestrator'ı run oturumunda bir tur danışır (`renderTriagePrompt`: görev, rapor özeti, her ölçütün kanıt durumu, rolün yapamayacağı komutlar için harness notu, atlanan kontroller, kalan retry). Orchestrator turları (steer danışması ve triyaj) sıralanır. Karar `task_triage` aracıyla verilir ve `DelegationPort.triage` tarafından denetlenir: yalnız triyajdaki görev; `accept` yalnız hiçbir dosya değiştirmemiş salt okunur görevde (explorer, `rca-only` debugger) ve en az bir ölçüt kanıtlı kalacak şekilde `waive_criteria` ile; `retry` yalnız retry hakkı varsa; `fail`. `task_spawn` bu turda da kabul edilir, eklenen görevler steer ile aynı revizyon/onay yolundan geçer. Sonuç:
...
- `retry`: yeni attempt; delta packet `Previous attempt … reported partial: <özet>`, `Orchestrator guidance: …`, kanıtsız ölçütler, atlanan kontroller ve çözülmemişler notlarını taşır.
