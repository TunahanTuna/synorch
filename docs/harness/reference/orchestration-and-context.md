...
| `scoped-dir` | Git değil, veya kullanıcı değişikliği owned yollarla çakışıyor, veya packet bunu istiyor | Yerinde yazma. Attempt öncesi **tüm ağacın** anlık görüntüsü alınır (SEC-M1): ignore edilen dosyalar, linkler, `.synorch/` ve kökteki `.git/config` + `.git/hooks/` dahil; yalnız `node_modules` ve `.git`'in geri kalanı atlanır. Sınırlar `SCOPED_SNAPSHOT_LIMITS`: en çok 200 000 girdi (aşılırsa scoped-dir `sandbox_insufficient` ile reddedilir), 4 MiB'a kadar dosyalar içerik hash'iyle, daha büyükleri boyut+mtime ile karşılaştırılır (bir görüntüde toplam 512 MiB hash), geri alma için en çok 256 MiB içerik saklanır (git'te HEAD içeriği digest'i tutuyorsa yedek olarak kullanılır). Görüntü `<worktrees>/<project-id>/<attempt-id>.scoped/` altında kalıcıdır (`manifest.json`, `owner.json`, `state.json`, `blobs/`). Farklı görevlerin scoped-dir attempt'leri **aynı anda çalışmaz**: paylaşılan dizinde bir değişiklik iki yazardan birine atfedilemez, bu yüzden ikinci attempt birincisi dispose edilene kadar bekler (aynı görevin revise attempt'i beklemez). Başka bir workspace'in bu sırada ana ağaca integrate ettiği yol, içeriği integrate edilenle aynıysa bu attempt'e atfedilmez. `revert()` owned olsun olmasın her değişikliği geri alır; yalnız görevin salt okunur girdileri (`read_paths`, `context.sources`) owned değilse yerinde bırakılır, çünkü onların değişikliği tazelik denetiminin kullanıcıya atfettiği kaynak değişikliğidir. |
...
**Artifact.** `changeSet()` değişen her yol için `{path, before, after, content}` üretir; `before/after` ham baytların `sha256`'sıdır. Kanonik JSON'un `sha256`'sı artifact digest'idir ve blob olarak saklanır. Değişiklik tespiti worktree'de `git status --porcelain -z --untracked-files=all --no-renames` ile, scoped-dir'de (git olsun olmasın) ignore edilen dosyaları da kapsayan ağaç görüntüsüyle yapılır. Değişen yol bir link veya özel dosyaysa ya da workspace yolu olarak ifade edilemiyorsa `ChangeSet.unsafe`'e girer; sessizce düşürülmez.
...
2. `changed ⊆ owned` ve forbidden/rezerve yol yok; `unsafe` boş (link ve özel dosya hiç integrate edilmez); değilse `policy_denied`.
3. **Link güvenliği (SEC-H4).** Worktree modunda artifact bir `.gitignore` değiştiriyorsa, ana çalışma alanında şu an ignore edilen ve yeni kurallarla (worktree'de `git check-ignore --no-index`) açığa çıkacak her giriş linkler için taranır; açığa çıkan link owned **ve** gerçek yolu workspace içinde değilse `policy_denied`. Ardından her hedef bayt yazılmadan önce `resolveLinkSafeTarget` ile segment segment çözülür: `lstat` link/junction görürse, `realpath` başka yere çözülürse, ara segment dizin değilse veya dosyanın birden çok hard link'i varsa tüm artifact reddedilir (kısmi integrate yok). Aynı çözümleme `seed` ve `revert`'te de kullanılır; revert ve kurtarma rezerve yolları geri yükleyebilir ama linkin içinden asla yazmaz (yazamadığı yolu raporlar).
4. Worktree modunda ana çalışma alanındaki her hedefin mevcut digest'i `before` ile eşleşmeli; kullanıcının bu arada yaptığı düzenleme veya aynı yoldaki untracked dosyası çakışma olarak raporlanır ve hiçbir dosya ezilmez. Sonra dosyalar atomik (temp + rename, parent oluşturulduktan sonra yeniden çözülerek) yazılır veya silinir.
5. Scoped-dir'de değişiklik zaten yerindedir; yalnız doğrulama yapılır ve kalıcı görüntü `integrated` olarak işaretlenir (kurtarma onu geri almaz).
6. Başarılı integrate sonrası WorkerManager run günlüğüne `task/integrated { task_id, attempt_id, artifact_digest, paths }` yazar; `completed` geçişi bundan sonra gelir.

`seed()` bir revise attempt'ini önceki artifact'tan başlatır; yeni artifact kümülatiftir.

**Çökme sonrası budama (SEC-M3).** Her worktree'nin yanında `<attempt-id>.owner.json`, her scoped-dir görüntüsünde `owner.json` (`pid`, `host`, `workspace_root`) tutulur. `pruneOrphanedAttempts({ worktreesRoot, projectId, workspaceRoot })` sahibi yaşayan (aynı host'ta süreci çalışan, bu süreç veya başka host) hiçbir şeye dokunmaz; sahibi ölmüş worktree'yi `git worktree remove --force` (olmazsa `rm` + `git worktree prune`) ile kaldırır; çökmüş ve integrate edilmemiş scoped-dir attempt'inin **owned** yollarındaki yarım yazmaları kalıcı görüntüden geri alır, owned dışı değişiklikleri (kullanıcı o arada düzenlemiş olabilir) `leftInPlace` olarak raporlar ve görüntüyü siler. Sahip kaydı olmayan dizinler `unknown` olarak bırakılır. Runtime `recover()` ve `syn doctor --runtime` (`store` sonucu) bunu çağırır.
...
- **Maliyet.** Scoped-dir (git olsun olmasın) ağacın tamamını (`node_modules` ve `.git`'in hook/config dışı kısmı hariç) oluşturmada ve her `changeSet`'te okur, 256 MiB'a kadar içeriği attempt dizinine kopyalar; büyük depolarda pahalıdır ve `node_modules` içindeki değişiklikler görülmez. Worktree oluşturma + kaldırma, bu Windows makinesinde küçük test depolarında yaklaşık 0,3–0,6 s sürdü; büyük depolar ve `node_modules` kurulumu ölçülmedi.
