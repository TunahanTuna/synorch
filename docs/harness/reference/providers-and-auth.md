...
- **Kimlik kaynağı zorlaması (SEC-M2):** `system/init.apiKeySource` abonelik girişini (`oauth` → `subscription`) bildirmezse tur `forbidden` ile reddedilir, `backend_init` yayımlanmaz, alt süreç durdurulur ve hiçbir araç çalışmaz. Yalnız kullanıcı yapılandırmasındaki açık opt-in (`adapters: [{ id: claude-code, kind: claude-code, allow_non_subscription_auth: true }]`) diğer kaynakları kabul eder; kaynak yine `backend_init.auth_source` ile gösterilir.
