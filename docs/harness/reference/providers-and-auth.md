...
- **Ortam (SEC-M2):** Alt süreç ortamı `BackendSessionOptions.env`'dir; `BRIDGE_STRIPPED_ENV` adları (`ANTHROPIC_API_KEY`, `ANTHROPIC_AUTH_TOKEN`, `ANTHROPIC_BASE_URL`, `CLAUDE_CODE_OAUTH_TOKEN`, `CLAUDE_CODE_USE_BEDROCK`, `CLAUDE_CODE_USE_VERTEX`, `AWS_BEARER_TOKEN_BEDROCK`, `OPENAI_API_KEY`, `CODEX_API_KEY`) ve `BRIDGE_STRIPPED_ENV_PREFIXES` ile başlayan her ad (`ANTHROPIC_*`, `CLAUDE_CODE_USE_*`) her platformda büyük/küçük harf duyarsız silinir (`isBridgeStrippedEnvName`). Böylece miras alınan Bedrock/Vertex/endpoint/token ortamı aboneliği sessizce bir bulut hesabına veya API key'e çeviremez.
- **Kimlik kaynağı zorlaması (SEC-M2):** `system/init.apiKeySource` abonelik girişini (`none` veya `oauth` → `subscription`) bildirmezse tur `forbidden` ile reddedilir, `backend_init` yayımlanmaz, alt süreç durdurulur ve hiçbir araç çalışmaz. Yalnız kullanıcı yapılandırmasındaki açık opt-in (`adapters: [{ id: claude-code, kind: claude-code, allow_non_subscription_auth: true }]`) diğer kaynakları kabul eder; kaynak yine `backend_init.auth_source` ile gösterilir. **Doğrulandı (2026-09-24, Claude Code 2.1.281):** claude.ai aboneliğiyle giriş yapmış `claude` `apiKeySource: "none"` bildirir; köprü tüm anahtar/token env'lerini temizlediği için `none` = abonelik girişi. API anahtarı kaynakları (`user`, `project`, `org`, `temporary`, `ANTHROPIC_API_KEY`, `apiKeyHelper`, `/login managed key`, adında key/helper/token geçen her değer) → `api-key`; eksik/tanınmayan → `unknown`; ikisi de opt-in olmadan reddedilir. Hiç giriş yapılmamışsa model isteği `Not logged in · Please run /login` ile düşer → `auth_expired` + "`claude`, ardından `/login`" yönlendirmesi.
- **MCP ön-kimlik sınırı (SEC-L3):** Kimlik doğrulanmamış bağlantı en fazla `MCP_PREAUTH_MAX_BYTES` (1024) bayt tek satır gönderebilir, doğrulanmış bağlantının satırı en fazla `MCP_LINE_MAX_BYTES` (16 MiB); aşan bağlantı arabelleği büyütülmeden kapatılır.
...
Güvenlik regresyonları: `harness-providers-bridge`: "SEC-M2 bridge env strips Bedrock, Vertex, base-url, OAuth-token and every ANTHROPIC_* / CLAUDE_CODE_USE_* name", "SEC-M2 bridge refuses a turn whose auth source is not the subscription login unless the user opted in", "SEC-L3 MCP bridge drops a connection whose pre-auth line exceeds the cap"; `harness-security-config`: "SEC-C1 …" (repo yapılandırması endpoint/route seçemez, `base_url` sabitlemesi).
