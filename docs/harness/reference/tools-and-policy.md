...
- **Windows shim çözümü (`tools/windows-launch.ts`, `planLaunch`):** policy ve sınıflandırıcı özgün argv'yi görür; çözüm yalnız onaylanmış argv'nin *nasıl* başlatılacağını belirler. POSIX'te argv değişmeden spawn edilir.
  1. Program PATH + PATHEXT ile (yalnız `.com/.exe/.bat/.cmd`; geçerli dizin aranmaz, dizin parçası olan ad `cwd`'ye göre) gerçek dosyasına çözülür; yerel çalıştırılabilir mutlak yoluyla spawn edilir. Bulunamayan program eskisi gibi `spawn-failed` olur.
