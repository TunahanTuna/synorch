...
const PLAIN_SETTINGS: readonly SettingDefinition[] = [
  ...
  { key: "ui.welcome.logo", kind: "enum", choices: WELCOME_LOGO_CHOICES, scope: "user", description: "welcome logo: on/art (the Synorch mark as colour art), glyph (text mark), off, or custom (<synorch home>/logo.txt)", fallback: "on", yamlPath: ["ui", "welcome", "logo"] },
  ...
  { key: "skills.include_claude", kind: "boolean", scope: "user", description: "also use Claude Code's own skills and commands (~/.claude/skills, ~/.claude/commands)", fallback: "true", yamlPath: ["skills", "include_claude"] },
  { key: "plugins.include_claude", kind: "boolean", scope: "user", description: "also use the plugins enabled in Claude Code (read in place, never changed)", fallback: "true", yamlPath: ["plugins", "include_claude"] },
