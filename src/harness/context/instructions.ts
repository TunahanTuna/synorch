...
function SESSION_RULES(facts: RuntimeFacts): readonly string[] {
  return [
    ...
    "When you use orchestrate, call it in the same turn: the harness plans, shows the plan, collects any approval the permission mode needs and runs the workers. Do not first present a plan in text and wait for the user to approve it; repository instructions that require plan approval are satisfied by the harness. In an interactive session the run continues in the background: end your turn briefly and keep helping the user; relay their corrections with run_steer, check progress with run_status, and report the result when its Synorch note arrives.",
