import type { AnyModelAdapter, AuthMethodKind, CapabilityLevel, RouteRule } from "../contracts/index.ts";
...
export interface CatalogInput {
  ...
  listed?(adapter: AnyModelAdapter): Promise<readonly string[] | undefined>;
}
...
export async function buildModelCatalog(input: CatalogInput): Promise<CatalogModel[]> {
  ...
  for (const adapter of adapters) {
    ...
    const push = (model: string, source: CatalogModel["source"], label?: string): void => {
    ...
    for (const model of listed ?? []) push(model, "provider");
 ...
 */
export async function fetchCodexModels(fetchImpl: FetchLike, headers: Headers, signal: AbortSignal, baseUrl = "https://chatgpt.com/backend-api/codex"): Promise<readonly string[] | undefined> {
  ...
  try {
    ...
    const ids = list
      .map((entry) => (typeof entry === "string" ? entry : typeof entry === "object" && entry !== null ? ((entry as Record<string, unknown>).slug ?? (entry as Record<string, unknown>).id) : undefined))
      .filter((id): id is string => typeof id === "string" && /^[A-Za-z0-9._:-]{1,120}$/.test(id));
