...
async function runBoard(columns: number, rows: number): Promise<{ frames: Frame[]; metrics: Metrics }> {
  ...
  await tui.start({ workspaceRoot: "/work/demo", gitBranch: "main", policyMode: "autonomous", routes: [], sandboxEnforcement: "full", notices: [], version: "0.3.0", model: "sol-large", permissionMode: "auto" });
  const frames: Frame[] = [];
  tui.flush();
  tui.setBoard(BOARD);
  tui.flush();
